...
export default function CreateAssignment({ onCreated }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const schema = Yup.object().shape({
      ...
      //file_path: Yup.mixed().required("Attachment is required"),
    });
    try {
      await schema.validate({ course_id: courseId, title, description, due_date: dueDate, max_marks: maxMarks, attachment: file }, { abortEarly: false });
    } catch (err: any) {
    ...
    try {
      ...
      if (file) {
        body.append("file_path", file);
      }

      const res = await fetch(`${apiBaseUrl}/course-assignments`, {
        method: "POST",
        headers: {
