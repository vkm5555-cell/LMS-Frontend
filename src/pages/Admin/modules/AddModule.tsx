...
export default function AddModule() {
    ...
    try {
      ...
      if (data.success === false) {
        if (data.error_code === 300) {
          setMessage(data.message || "Permission denied");
          navigate("/admin");
        }
        if (data.error_code === 301) {
          setMessage(data.message || "Module already exists");
  ...
  return (
    <>
      <PageMeta title="BBD ED LMS | Add Module" description="BBD ED LMS - Add Module" />
      <PageBreadcrumb pageTitle="Add Module" />

      <div className="space-y-6">
        <ComponentCard title="Add Module">
          {message && (
            <p
