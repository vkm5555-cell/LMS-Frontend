...
export default function EditSubCourseCategory() {
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const allRes = await fetch(`${apiBaseUrl}/course-categories`, {
          headers: {
        ...
        setCategories(Array.isArray(allData.data) ? allData.data : []);
        ...
        if (data.success && data.data) {
          setFormData({
            ...
            parent_category_id: data.data.parent_category_id ? String(data.data.parent_category_id) : ""
          });
        } else {
          setMessage(data.message || "Failed to fetch sub category");
        }
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success === false) {
        setMessage(data.message || "Failed to update sub course category");
        if (data.error_code === 403) {
          navigate("/permission-denied");
        }
      } else {
        setMessage(data.message || "Sub category updated successfully!");
        setTimeout(() => {
  ...
  return (
    <>
      <PageMeta title="BBDU LMS | Edit Sub Course Category" description="BBDU LMS - Edit Sub Course Category" />
      <PageBreadcrumb pageTitle="Edit Sub Course Category" />
      <div className="space-y-6">
        <ComponentCard title="Edit Sub Course Category">
          {message && (
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              >
                ...
                {categories
                  .filter((cat) => String(cat.id) !== String(id))
                  .map((cat) => (
            ...
            >
              {loading ? "Saving..." : "Update Sub Course Category"}
            </button>
          </form>
        </ComponentCard>
      </div>
