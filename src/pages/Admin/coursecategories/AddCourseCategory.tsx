...
export default function AddCourseCategory() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success === false) {
        setMessage(data.message || "Failed to add course category");
        if (data.error_code === 403) {
          navigate("/permission-denied");
        }
        setFormData({ name: "", description: "", keyword: "", parent_category_id: null });
      } else {
  ...
  return (
    <>
      <PageMeta title="BBDU LMS | Add Course Category" description="BBDU LMS - Add Course Category" />
      <PageBreadcrumb pageTitle="Add Course Category" />
      <div className="space-y-6">
        <ComponentCard title="Add Course Category">
          {message && (
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            >
              {loading ? "Saving..." : "Add Course Category"}
            </button>
          </form>
        </ComponentCard>
      </div>
